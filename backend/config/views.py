...
def _parse_date(value: str):
    """
    Parse common CSV date formats. Adjust if your CSV uses something else.
...
def _is_valid_row(date, station_id, temp_c, precip_mm):
    """
    Validation rules (tweak as needed):
    - date required
    - station_id required (non-empty)
    - temp_c required and within plausible range
    - precip_mm required and non-negative
    """
    if date is None:
        return False
...
def import_csv(request):
    ...
    # Expect columns: date, station_id, temp_c, precip_mm
    ...
    for raw in reader:
        ...
        # Always load into staging (dirty allowed)
        staging_rows.append(
            StagingMeasurement(
                date=date,
    ...
    with transaction.atomic():
        ...
        rows_upserted = 0
        if valid_clean_rows:
            # Django 5.2 supports bulk upsert via update_conflicts
            ...
            # "upserted" = how many valid rows we attempted to upsert
